...
export class MemoramaControllerService {
  ...
  public verifyTime(): void{
    if(this.chronometerService.minutes >= 1){
      ...
      this.chronometerService.updated.next({gameOver: this.isGameOver, isPaused: this.isPaused});
    }
  }
  ...
  public play(renderer: Renderer2, event: any){
    if (this.currentMove < 2 && !this.isGameOver && !this.isPaused) {
      ...
      if ((!this.selectedCards[0] || this.selectedCards[0].card !== card) && !content.classList.contains('active')) {
  ...
  private cardsAreEven(): boolean{
    return this.selectedCards[0].card.querySelectorAll('.face')[0].textContent === this.selectedCards[1].card.querySelectorAll('.face')[0].textContent;
  }

  public isTheEndGame(): boolean{
    return this.cards.every(card => card.content.classList.contains('active'));
  }
